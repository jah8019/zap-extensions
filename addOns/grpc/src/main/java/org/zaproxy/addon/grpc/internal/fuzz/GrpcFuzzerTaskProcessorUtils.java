...
public class GrpcFuzzerTaskProcessorUtils {

    private static final Logger LOGGER = LogManager.getLogger(GrpcFuzzerTaskProcessorUtils.class);

    private static final int PLUGIN_ID = 50002;

    private final GrpcFuzzer grpcFuzzer;
    ...
    protected GrpcFuzzerTaskProcessorUtils(
            GrpcFuzzer grpcFuzzer,
            HttpMessage originalMessage,
            long taskId,
            List<Object> payloads) {
        this(grpcFuzzer, originalMessage, taskId, payloads, null, null);
    }
    ...
    public void setCurrentProcessorName(String name) {
        processorName = name;
    }
    ...
    public void increaseErrorCount(String reason) {
        grpcFuzzer.increaseErrorCount(taskId, processorName, reason);
    }
    ...
    public boolean sendMessage(HttpMessage message, boolean followRedirects) {
        try {
            grpcFuzzer.getHttpSender().sendAndReceive(message, followRedirects);
            grpcFuzzer.messageSent(taskId, message);
            return true;
        } catch (IOException e) {
    ...
    public void addMessageToResults(
        ...
        grpcFuzzer.fuzzResultAvailable(result);
    }
